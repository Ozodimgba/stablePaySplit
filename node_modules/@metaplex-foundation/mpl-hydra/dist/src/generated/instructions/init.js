"use strict";
/**
 * This code was AUTOGENERATED using the kinobi library.
 * Please DO NOT EDIT THIS FILE, instead use visitors
 * to add features, then rerun kinobi to update it.
 *
 * @see https://github.com/metaplex-foundation/kinobi
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.init = exports.getInitInstructionDataSerializer = void 0;
const umi_1 = require("@metaplex-foundation/umi");
const serializers_1 = require("@metaplex-foundation/umi/serializers");
const hooked_1 = require("../../hooked");
const accounts_1 = require("../accounts");
const shared_1 = require("../shared");
const types_1 = require("../types");
function getInitInstructionDataSerializer(_context = {}) {
    return (0, serializers_1.mapSerializer)((0, serializers_1.struct)([
        ['discriminator', (0, serializers_1.array)((0, serializers_1.u8)(), { size: 8 })],
        ['bumpSeed', (0, serializers_1.u8)()],
        ['nativeAccountBumpSeed', (0, serializers_1.u8)()],
        ['name', (0, serializers_1.string)()],
        ['totalShares', (0, serializers_1.u64)()],
        ['model', (0, types_1.getMembershipModelSerializer)()],
    ], { description: 'InitInstructionData' }), (value) => ({
        ...value,
        discriminator: [172, 5, 165, 143, 86, 159, 50, 237],
    }));
}
exports.getInitInstructionDataSerializer = getInitInstructionDataSerializer;
// Instruction.
function init(context, input) {
    const signers = [];
    const keys = [];
    // Program ID.
    const programId = context.programs.getPublicKey('mplHydra', 'hyDQ4Nz1eYyegS6JfenyKwKzYxRsCWCriYSAjtzP4Vg');
    // Resolved inputs.
    const resolvedAccounts = {};
    const resolvingArgs = {};
    (0, shared_1.addObjectProperty)(resolvedAccounts, 'authority', input.authority
        ? [input.authority, true]
        : [context.identity, true]);
    (0, shared_1.addObjectProperty)(resolvedAccounts, 'fanout', input.fanout
        ? [input.fanout, true]
        : [(0, accounts_1.findFanoutPda)(context, { name: input.name }), true]);
    (0, shared_1.addObjectProperty)(resolvedAccounts, 'holdingAccount', input.holdingAccount
        ? [input.holdingAccount, true]
        : [
            (0, hooked_1.findFanoutNativeAccountPda)(context, {
                fanout: (0, umi_1.publicKey)(resolvedAccounts.fanout[0], false),
            }),
            true,
        ]);
    (0, shared_1.addObjectProperty)(resolvedAccounts, 'systemProgram', input.systemProgram
        ? [input.systemProgram, false]
        : [
            context.programs.getPublicKey('splSystem', '11111111111111111111111111111111'),
            false,
        ]);
    (0, shared_1.addObjectProperty)(resolvedAccounts, 'membershipMint', input.membershipMint
        ? [input.membershipMint, true]
        : [
            (0, umi_1.publicKey)('So11111111111111111111111111111111111111112'),
            true,
        ]);
    (0, shared_1.addObjectProperty)(resolvedAccounts, 'rent', input.rent
        ? [input.rent, false]
        : [
            (0, umi_1.publicKey)('SysvarRent111111111111111111111111111111111'),
            false,
        ]);
    (0, shared_1.addObjectProperty)(resolvedAccounts, 'tokenProgram', input.tokenProgram
        ? [input.tokenProgram, false]
        : [
            context.programs.getPublicKey('splToken', 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'),
            false,
        ]);
    (0, shared_1.addObjectProperty)(resolvingArgs, 'bumpSeed', input.bumpSeed ?? resolvedAccounts.fanout[0][1]);
    (0, shared_1.addObjectProperty)(resolvingArgs, 'nativeAccountBumpSeed', input.nativeAccountBumpSeed ?? resolvedAccounts.holdingAccount[0][1]);
    const resolvedArgs = { ...input, ...resolvingArgs };
    (0, shared_1.addAccountMeta)(keys, signers, resolvedAccounts.authority, false);
    (0, shared_1.addAccountMeta)(keys, signers, resolvedAccounts.fanout, false);
    (0, shared_1.addAccountMeta)(keys, signers, resolvedAccounts.holdingAccount, false);
    (0, shared_1.addAccountMeta)(keys, signers, resolvedAccounts.systemProgram, false);
    (0, shared_1.addAccountMeta)(keys, signers, resolvedAccounts.membershipMint, false);
    (0, shared_1.addAccountMeta)(keys, signers, resolvedAccounts.rent, false);
    (0, shared_1.addAccountMeta)(keys, signers, resolvedAccounts.tokenProgram, false);
    // Data.
    const data = getInitInstructionDataSerializer().serialize(resolvedArgs);
    // Bytes Created On Chain.
    const bytesCreatedOnChain = 557;
    return (0, umi_1.transactionBuilder)([
        { instruction: { keys, programId, data }, signers, bytesCreatedOnChain },
    ]);
}
exports.init = init;
//# sourceMappingURL=init.js.map